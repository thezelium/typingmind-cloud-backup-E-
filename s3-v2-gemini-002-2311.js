...
function importDataToStorage(data) {
  return new Promise((resolve, reject) => {
    ...
    const tryReviveBlob = (value) => {
        // Caso 1: Nuovo formato con __is_blob (per i backup futuri)
        if (value && typeof value === 'object' && value.__is_blob === true && value.data) {
            return dataURLToBlob(value.data);
        }
        
        // Caso 2: Vecchio formato (Buffer/Array di byte)
        // Spesso i file binari finiscono in JSON come { type: 'Buffer', data: [12, 255, ...] } 
        // oppure oggetti generici con proprietà binaria
        if (value && typeof value === 'object') {
            // Controllo se sembra un array di byte trasformato in oggetto
            if (value.type === 'Buffer' && Array.isArray(value.data)) {
                return new Blob([new Uint8Array(value.data)]);
            }
            // Controllo generico per oggetti che hanno 'body' o 'data' che sembrano stream
            if (value.type && (value.type.includes('application/') || value.type.includes('image/')) && Array.isArray(value.data)) {
          ...
          Object.entries(data.indexedDB).forEach(([key, value]) => {
            if (!preserveKeys.includes(key)) {
              try {
                ...
                // 1. Prova a convertire dai formati noti (New Blob o Old Buffer)
                valueToStore = tryReviveBlob(value);

                // 2. Se è ancora un oggetto semplice e non un Blob, proviamo a vedere se è una stringa JSON
                if (!(valueToStore instanceof Blob) && typeof valueToStore === "string" && (valueToStore.startsWith("{") || valueToStore.startsWith("["))) {
                  try {
                    const parsed = JSON.parse(valueToStore);
                    // Se il parsing rivela un oggetto che sembra un file, riproviamo la conversione
                    const revived = tryReviveBlob(parsed);
                    valueToStore = revived !== parsed ? revived : parsed;
                  } catch (e) {
                    // Ignora, era solo una stringa
                  }
                }
                
                // 3. Caso specifico per TypingMind: a volte salvano i file non come Blob puri ma oggetti 
                // Se valueToStore è un oggetto che ha proprietà 'type' e 'size' ma non è un Blob,
                // potrebbe essere corrotto. Tuttavia, senza i dati binari grezzi, non possiamo farci molto.
                // Salviamo quello che abbiamo.
                ...
                logToConsole("error", `Error restoring IndexedDB setting ${key}:`, error);
              }
            }
          });
        };
...
// --- HELPER FUNCTIONS PER GLI ALLEGATI (MANCANTI) ---
