...
async function downloadFromS3(key) {
  ...
  try {
    ...
    logToConsole("download", `Starting download of ${key}...`);
    const response = await s3.getObject(params).promise();
    
    if (!response.Body || response.Body.length === 0) {
      throw new Error(`Downloaded file ${key} is empty`);
    }
    
    logToConsole("success", `Downloaded ${key}`, {
...
async function restoreFromBackup(key) {
  ...
  try {
    ...
    logToConsole("info", `Backup downloaded successfully`, {
      size: backup.data.length,
      isZip: key.endsWith(".zip"),
    });
    ...
    if (key.endsWith(".zip")) {
      ...
      logToConsole("info", `Extracting ${jsonFile} from ZIP...`);
      backupContent = await zip.file(jsonFile).async("uint8array");
      logToConsole("info", `Extracted content size: ${backupContent.length} bytes`);
    } else {
      backupContent = backup.data;
    }
    
    if (!backupContent || backupContent.length === 0) {
      throw new Error("Backup content is empty after extraction");
    }
    ...
    logToConsole("info", "Decrypted content info:", {
      type: typeof decryptedContent,
      length: decryptedContent.length,
      preview: decryptedContent.slice(0, 100),
    });
